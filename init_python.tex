...
\end{frame}
\begin{frame}[fragile]
\frametitle{Exo 1 : chaînes de caractères}
\begin{itemize} 
  \item ecrire une fonction qui prend une chaine $s$ et qui retourne la chaine a l'envers
  \item ecrire une fonction qui norme les chaines : pas de majuscule, d'espace de ponctuation ou d'apostrophe 
  \item ecrire une fonction qui verifie qu'une chaine est un palindrome, après normage.
  \begin{itemize}
    \item  Laval, ici, Laval
    \item tu l'as trop ecrase César ce port salut
    \item Esope reste ici et se repose.
  \end{itemize}
\end{itemize}
\end{frame}
\begin{frame}[fragile]
\frametitle{Exo 2 : énigme mathematique}
\begin{itemize} 
 \item je cherche un nombre entier $p$, tel que $p-1$ soit un carré, et $p+1$ un cube
 \item indice : il est entre 1 et 100.
 \item écrire une fonction est\_un\_cube en utilisant la fontion racine cubique donnee.
...
\begin{itemize} 
   \item on souhaite faire rebondir une sphere avec la bibliothèque VTK
\end{itemize}
\end{frame}
