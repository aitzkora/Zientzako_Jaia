...
\begin{frame}[fragile]
\frametitle{Définition de la Programmation}
\begin{itemize}
  \item En partant d'un \textbf{algorithme} qui réalise une tâche donnée, on écrit un logiciel ou un programme à l'aide d' un langage
  informatique, qui lorsque qu'on l'execute sur un ordinater va effectivement réaliser cette tâche.
  \item le programme prend souvent des données en entrée, réalise un traitement dessus, et fournit des données en sortie. C'est la
  valeur ajouté d'un programme
\end{itemize}
\begin{center}
\includegraphics[width=0.3\linewidth]{dik2.pdf}
\end{center}
\end{frame}
...
\begin{itemize}
  \item \python est un langage interprété : on peut modifier
  dynamiquement (à la volée) le code du programme.
  \item la structure du programme se gère grâce à des indentations : attention à aligner
  les blocs avec le même nombre d'espace
\vspace{1cm}
\begin{columns}
...
\begin{itemize}
 ...
 programme qu'on écrit soit un programme affichant ``bonjour, monde''\footnote{Hello, World dans la langue de Shakespeare}
\item dans la console de {\bf spyder}, on ecrira
\begin{lstlisting}
print 'bonjour, Monde'
\end{lstlisting}
\item ca doit ressembler à quelque chose comme
\begin{center}
...
\end{frame}
\begin{frame}[fragile]
\frametitle{Code César et messages secret}
\begin{itemize} 
  \item con
\end{itemize}
\end{frame}
...
\begin{itemize} 
   \item on souhaite faire rebondir une sphére avec la bibliothèque VTK
\end{itemize}
\end{frame}
